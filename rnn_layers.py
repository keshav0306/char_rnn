...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    a_next = b + prev_h @ Wh + x @ Wx
    next_h = np.tanh(a_next)
    cache = (x, prev_h, Wx, Wh, b, next_h, a_next)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, next_h, a_next = cache
    ##############################################################################
    ...
    # let a be the N * H matrix representing the gradients of the tanh function
...
def rnn_backward(dh, cache):
    ...
    N, T, H, D = dh.shape[0], dh.shape[1], dh.shape[2], cache[0][0].shape[1]
    ##############################################################################
    ...
    for i in range(T-1, -1, -1):
      dnext_h = dnext_h_new + dh[:, i, :]
      dx[:, i, :], dnext_h_new, dWx_new, dWh_new, db_new = rnn_step_backward(dnext_h, cache[i])
      dWx += dWx_new
      dWh += dWh_new
      db += db_new
      if(i == 0):
...
def word_embedding_backward(dout, cache):
    ...
    for i in range(V):
      ...
      dW[i] = np.sum(dout[indices], axis = 0)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # assume first H of a contain computations for input, next for forget, next for output, next for block 
    ai = sigmoid(a[:,:H])
    af = sigmoid(a[:,H:2*H])
    ao = sigmoid(a[:,2*H:3*H])
    ag = tanh(a[:,3*H:4*H])
    next_c = prev_c * af + ai * ag
    next_h = tanh(next_c) * ao
    cache = (x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, b, a, ai, af, ao, ag, next_c, next_h = cache
    dactnext_c = dnext_c + dnext_h * ao * (1 - (tanh(next_c) ** 2))
    dprev_c = dactnext_c * af
    dao = dnext_h * tanh(next_c)
    daf = dactnext_c * prev_c
    dag = dactnext_c * ai
    dai = dactnext_c * ag
    
    dao_before = dao * ao * (1 - ao)
    daf_before = daf * af * (1 - af)
    dai_before = dai * ai * (1 - ai)
    dag_before = dag * (1 - (ag ** 2))

    da = np.concatenate([dai_before, daf_before, dao_before, dag_before], axis = 1) # (N * 4H)
    db = np.sum(da, axis = 0)
    dWx = x.T @ da
    dWh = prev_h.T @ da
    dx = da @ Wx.T
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        next_h, next_c, cache_curr = lstm_step_forward(x[:,i,:], prev_h, prev_c, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T):
      ...
      dx[:,T-i-1,:], dnext_h, dprev_c, dwx, dwh, dB = lstm_step_backward(dnext_h, dnext_c, cache[T-1-i])
      dWx += dwx
      dWh += dwh
      db += dB
      dnext_c = dprev_c
